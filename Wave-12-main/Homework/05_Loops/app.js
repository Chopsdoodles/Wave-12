...
const storeName = "QuickShop"; // Const variable, the store name does not need to change.
const budget = 100; // Budget is fixed , but I guess this could be a let  variable if you wanted to increase the budget.
const itemLimit = 30; //
const discountThreshold = 80; // Const variable  I assume because the word "threshold" here.
const discountRate = 0.10; // Const variable, the discount rate would stay the same.
let cartTotal = 0; // Let variable because we expect this number to increase or decrease.
let itemsAdded = 0;// Let variable because the items added would change the value.
let itemsSkipped =0; // let variable because we dont know how many item will be skipped based on whats left of budget.
...
if (cartTotal > discountThreshold) {
    ...
    console.log("💰 Total afer discount: $" + cartTotal)
} else {
...
for (let i = 1; i <= 6; i++) {
    let currentPrice;
    if (i === 1) {currentPrice = item1Price
    } else if ( i === 2) {currentPrice = item2Price
    } else if ( i === 3) { currentPrice = item3Price
    } else if ( i === 4) { currentPrice = item4Price
    } else if ( i === 5) { currentPrice = item5Price
    } else if ( i === 6) { currentPrice = item6Price
